---
title: "【図解】dbtとは？仕組み・実務でのシステム構成・主要コマンドを解説"
emoji: "🔧"
type: "tech"
topics:
  - "dbt"
...
データエンジニア界隈において、モダンデータスタックの中心的なツールとして完全に定着した感のある **dbt（data build tool）**。

界隈ではすっかりお馴染みとなり「今更感」も強いですが、自分自身の備忘録も兼ねて、dbtの概要から実務でのシステム構成、主要コマンド、実際のモデルの書き方まで、図解を交えて体系的にまとめておきます。
...
:::message
dbtはあくまで「SQLを組み立てて投げるツール」です。自らデータを分散処理するSpark等のフレームワークとは根本的に役割が異なります。
:::
...
| **`models/`** | SQLモデル（データ変換のロジック）とYAMLのテスト/スキーマ定義の格納場所 |
| **`tests/`** | SQLで直接書くSingular tests（単一テスト）。Generic tests（汎用テスト）では表現しきれないビジネスルールを検証 |
...
| **`dbt test`** | テスト（Generic tests + Singular tests）のみを実行 |
...
flowchart LR
    ...
    subgraph Prod["🚀 本番運用フェーズ（CI/CD・バッチ等）"]
        direction TB
        Build["dbt build\n(seed・run・test・snapshotを内包。\n依存関係順にノード単位でテストを行いながら\n安全に一括実行)"]
    end
...
開発時は上記のように個別のコマンドを叩いて確認することが多いですが、本番環境では **`dbt build`** コマンドを1つ叩くだけで、DAG（依存関係）の順序通りに処理を一括実行してくれます。
特筆すべきは、全モデルをまとめて作成した後にまとめてテストする（バッチ的な動作）**わけではない**という点です。「あるモデルを作成（run）したら、直後にそのモデルをテスト（test）し、合格した場合のみ下流モデルの作成に進む」という、ノード単位での厳密な挙動をしてくれます。
:::

:::message
**ノード（Node）とは？**
dbtにおける「ノード」とは、データパイプライン（依存関係のグラフ）を構成する「1つ1つの処理の単位」のことです。具体的には、1つのモデル（`.sql`）、1つのテスト、1つのシード（`.csv`）などがそれぞれ独立した「ノード」として扱われます。
`dbt build` は、このノードという最小単位ごとに「実行 ➔ 即座にテスト」を繰り返しながら安全に処理を進めていく仕組みになっています。
:::
...
- **`dbt build`**：モデルの実行 + テスト + シード + スナップショットを、依存関係に沿って実行する。本番のバッチジョブではこちらが基本

前述の通り、`dbt build` は **「モデルごとのrunとtestをセットで逐次実行する」** 挙動をします。これにより、「テストが失敗した瞬間に、そのモデルに依存する全ての下流モデルの実行（run）をスキップする」という強力なフェイルセーフが自動的に働き、不正なデータが下流のマートやBIツールに流れるリスクを水際で防ぐことができます。
:::
...
ステージングモデルは、ソースの生テーブルを**1対1で**クレンジング・整形するモデルです。
...
:::message
**【補足】QUALIFY句の方言について**
上記のSQLで使用している `QUALIFY` 句は、Snowflake、BigQuery、DatabricksなどのモダンDWHではサポートされている非常に便利で推奨される書き方です。
しかし標準SQL（ANSI SQL）ではないため、Amazon RedshiftやPostgreSQL、Athenaなどを使用している環境ではそのままではエラーになります。その場合は、サブクエリやCTE（共通テーブル式）内で `ROW_NUMBER()` を計算し、外側のクエリの `WHERE` 句で絞り込む標準的な書き方に変更してください。
:::
...
マートモデルは、ステージングモデルを結合・集計して、ビジネスユーザーが直接利用するテーブルを作るモデルです。
...
:::message
**メダリオンアーキテクチャについて**
データレイクハウス等で用いられる「Bronze / Silver / Gold」のデータ層分け（メダリオンアーキテクチャ）については、以下の記事で詳しく解説しています。本セクションの前提知識として参考にしてください。
:::
...
Generic tests（汎用テスト）だけでは表現できないビジネスルールは、SQLで直接テストを書くこともできます。テストのSQLは「**不正なレコードを返す**」ように書き、結果が0行なら合格です。
